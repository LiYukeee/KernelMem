...
for kp in "${kernel_path[@]}"; do
  log_name=$(basename "$kp" .py)
  command_list+=("python main_memory_latest.py $kp --gpu RTX5090 --server_type giteeai --model_name DeepSeek-V3 --round 20 --work_dir run --device 0 > run/log_${log_name}.log 2>&1")
done
